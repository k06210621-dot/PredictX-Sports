#!/usr/bin/env python3
"""
重新分析所有 MLB 已完賽事 — 使用即時 MLB Stats API + 天氣資料
"""
...
cur.execute("""
    ...
    WHERE t.league = 'MLB'
      AND g.status IN ('final', 'FINAL', 'finished')
...
print(f"Re-analyzing {len(games)} MLB completed games with live data + weather...\n")
...
engine.close()
elapsed = time.time() - start
print(f"\nDone! {success}/{len(games)} MLB games in {elapsed:.0f}s")