...
cur.execute("""
    ...
    WHERE g.status IN ('final', 'FINAL', 'finished')
...
print(f"Re-analyzing {len(games)} completed games...\n")
...
for idx, game in enumerate(games):
    ...
    try:
        ...
        print(f"[{idx+1}/{len(games)}] {league:4s} ✓ conf={conf} home_p={home_p:.3f} ({elapsed:.0f}s)")
    except Exception as e:
        elapsed = time.time() - start
        print(f"[{idx+1}/{len(games)}] {league:4s} ✗ Error: {e} ({elapsed:.0f}s)")

engine.close()
elapsed = time.time() - start
print(f"\nDone! {success}/{len(games)} games in {elapsed:.0f}s")