...
# 預設：Ollama Cloud + minimax-m3:cloud（NVIDIA minimax-m3 常 401，Ollama Cloud 穩定）
FALLBACK_LLM_URL = os.environ.get("FALLBACK_LLM_URL", "https://ollama.com/api/chat")
FALLBACK_LLM_MODEL = os.environ.get("FALLBACK_LLM_MODEL", "minimax-m3:cloud")
# 備援 API Key 獨立設定，若沒設則沿用主要 NOUS_API_KEY
FALLBACK_LLM_API_KEY = os.environ.get("FALLBACK_LLM_API_KEY", os.environ.get("OLLAMA_API_KEY", ""))
...
def _extract_rate(text, side="home"):
    ...
    # 🆕 [2026-08-31 精準修復] 根因：`主隊 ... 客隊` 配對跨越了歷史數據句子（如
    #   「主隊最大的心理籌碼。然而統一客場勝率67%...客隊勝率55%」），
    #   原本負向斷言 (?!主場|客場) 只擋「主隊緊鄰後一字元」，擋不住「中間隔好幾句
    #   夾雜客場勝率67%」，導致 pattern1 誤抓 home=67%, away=55%，方向檢測被繞過。
    # 修法：保留寬鬆的成對匹配（避免漏抓正常格式），但在匹配到後加「歷史數據過濾」：
    #   若匹配區段夾雜「主場/客場/戰績」等歷史關鍵字，判定為誤抓 → 放棄，繼續試下一組。
    pair_patterns = [
        # 中文版：主隊 X%，客隊 Y%（寬鬆，數字前允許各種描述）
        r"主隊[^\d]{0,30}?(\d+(?:\.\d+)?)\s*%[\s\S]{0,80}?客隊[^\d]{0,30}?(\d+(?:\.\d+)?)\s*%",
        r"客隊[^\d]{0,30}?(\d+(?:\.\d+)?)\s*%[\s\S]{0,80}?主隊[^\d]{0,30}?(\d+(?:\.\d+)?)\s*%",
        # 英文版：home/away X%
        r"home[^\d]{0,40}?(\d+(?:\.\d+)?)\s*%[\s\S]{0,80}?away[^\d]{0,40}?(\d+(?:\.\d+)?)\s*%",
        r"away[^\d]{0,40}?(\d+(?:\.\d+)?)\s*%[\s\S]{0,80}?home[^\d]{0,40}?(\d+(?:\.\d+)?)\s*%",
    ]
    for pat in pair_patterns:
        for m in re.finditer(pat, text.lower()):
            # 🆕 [2026-08-31] 歷史數據過濾：若匹配區段夾雜「主場/客場/戰績」等歷史關鍵字，
            # 判定為誤抓，跳過改試下一個匹配。
            matched_segment = m.group(0)
            has_history_kw = re.search(r"主場|客場|戰績|勝場|敗場", matched_segment)
            if has_history_kw:
                continue
    ...
    def get_game_features(self, game_id):
        ...
        if league and league.upper() == 'CPBL':
            try:
                ...
                # 🆕 [2026-08-21] Fallback：若 sportify.tw 失敗，從 cpbl_pitcher_pr 補讀
...
- summary 中必須明確寫出雙方先發的 K/9 與 BB/9 對比數字，並說明其對勝負的影響。"""
        ...
        if "MLB" in league_upper_check:
            ...
            home_advantage_note = (
                "\n- CPBL 主場勝率 ~47%（**客隊略佔優勢**，近 30 天主隊勝率僅 41%）。"
                "勿因「主場」一詞就預設主隊有利——以雙方實際數據對比為準。"
        ...
        if league and league.upper() == 'CPBL':
            ...
            cpbl_spec += "- 主場勝率約 47%（客隊略佔優勢，依實際對戰實力判斷）\n"
...
- 重要：若 Step 2 顯示客隊明顯佔優（如對手戰績、投手、打線都更好），即使主隊有主場，home_win_probability 也應 < 0.50
...
5. **禁止系統性傾向任一方向**：應根據對戰實力強弱決定傾向，而非「主場 = 加分」單一邏輯。CPBL 近 30 天客隊略佔優勢（主隊勝率僅 41%），勿預設主隊有利；若客隊數據明顯較強，請直接給客隊較高勝率
...
【反偏差自檢 (Anti-Bias Self-Check) — 強制執行】
完成分析後，請自我檢查並修正：

1. **若本場預測主隊勝率 > 0.55，但 Step 2 多數維度（戰績、打線、牛棚）顯示客隊佔優 → 必須將 h_prob 下修至 ≤ 0.50**
2. **若本場預測主隊勝率 < 0.45，但主隊多數維度佔優 → 必須將 h_prob 上修至 ≥ 0.50**
3. **h_prob 必須與 Step 2 的實質強弱推論方向一致**，而非反映「主場 = 加分」的口號
4. **比分方向必須與 h_prob 一致**（h_prob > 0.5 時比分主隊必須 > 客隊）
5. **summary 結語的強弱結論必須與 h_prob 方向一致**

**核心原則**：先發投手優異（單項）不足以覆蓋戰力 + 打線 + 牛棚 + 傷兵（多項）的整體劣勢，h_prob 必須反映「整體」而非「單項」。
    ...
    def analyze_game(self, game_id):
        ...
        if result and isinstance(result, dict):
            ...
            if has_all and has_summary:
                ...
                #   bucket 0(0.00)→3, 1(0.05)→4, 2(0.10)→6, 3(0.15)→6,
                #   4(0.20)→6, 5(0.25)→8, 6(0.30)→8, 7(0.35)→9,
                #   8(0.40)→9, 9(0.45)→9, 10+(0.50+)→10
                #   （3/4 於 2026-09-26 P0-2 由 7 下修 6，見下方實測依據）
                bucket_conf_map = {
                    0: 3, 1: 4, 2: 6, 3: 6,
                    4: 6, 5: 8, 6: 8, 7: 9,
                    8: 9, 9: 9, 10: 10,
                }
                # 🆕 [2026-09-26 P0-2] bucket 3(0.15)/4(0.20) 由 7 下修為 6。
                # 實測（ai_prediction_history 全期已結算）：
                #   gap 0.15 → 53.4% (n=283)、gap 0.20 → 56.4% (n=328)  ← 舊 map 給 7
                #   對照 gap 0.10 → 55.9% 給 6、gap 0.25 → 56.8% 給 8
                #   → 舊 map 在中段非單調，且 conf=7 整段實測僅 51.9% (n=104)，
                #     低於 conf=5 (55.5%) 與 conf=6 (63.5%)，屬真正「虛高信心」。
                # 註：原提案「調整加權 0.3/0.7 → 0.4/0.6」經實測否決 ——
                #   同一 gap 桶內，LLM 原始信心高低組命中率無差異（差異 -0.4 ~ -7.7pp），
                #   0.3 權重屬雜訊，提高權重只會放大雜訊；且 conf 8/9 實測 66.2%/95.7%
                #   確實具預測力，整體降權重會壓傷有效訊號。故僅修正此二桶。
