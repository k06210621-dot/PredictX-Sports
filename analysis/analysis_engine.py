...
{a_start}"""
        ...
        # 🆕 [2026-06-24] 信心理量規則（讓模型誠實自評）
        confidence_calibration = """
- **1-3**: 數據嚴重不足或兩隊實力極為接近，推論機率可能僅 50-55%。
...
- **9**: 極高把握（如 ace 對弱投），預期驗證率 85-90%。
- **10**: 史詩級優勢，僅限極少數情況（幾乎不打）。
- **重要：信心 7+ 必須真正「明顯佔優」才給**，不要為了用戶體驗而過度自信。"""
...
若 MLB 缺少先發投手資料，請在 confidence 自動降 2 級。
...
用 **加權平均** 計算 home_win_probability：
- 起點 0.50（五五波基準）
- 每個因子依權重與差值加減
- 加上主場優勢係數（{league_home_adv:.3f}）
...
4. 禁止系統性傾向客隊：實力接近時主隊勝率應在 0.52-0.55 區間
5. predicted_score 必須是具體字串如 "3-2"，不可是 "N/A" 或空值
6. radar_chart 必須有 6 個 categories 和對應 6 個數值（0-10），不得為空
7. key_factors 至少 4 個，**每個必須包含具體數字/球員名/百分比**
