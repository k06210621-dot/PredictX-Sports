...
# 🆕 一次性 DELETE endpoint：清除錯誤資料（已驗證 game_id 2c249ade... 為不存在於現實 NBA 6/14 的對戰）
# 寫死一次性 token（用過後可移除此 endpoint）
_ONE_TIME_ADMIN_TOKEN = "predictx-cleanup-2026-06-18"
@app.route('/api/admin/delete_game/<game_id>', methods=['DELETE'])
def admin_delete_game(game_id):
    admin_token = request.headers.get('X-Admin-Token', '')
    if admin_token != _ONE_TIME_ADMIN_TOKEN:
    ...
    try:
        ...
        cur.execute("""
            SELECT g.game_id, g.match_date,
                   th.english_name AS home_en, ta.english_name AS away_en
            FROM predictx.games g
            JOIN predictx.teams th ON g.home_team_id = th.team_id
            JOIN predictx.teams ta ON g.away_team_id = ta.team_id
        ...
        cur.execute("DELETE FROM predictx.game_status WHERE game_id = %s::uuid", (game_id,))
        deleted_status = cur.rowcount
        cur.execute("DELETE FROM predictx.games WHERE game_id = %s::uuid", (game_id,))
        deleted_game = cur.rowcount
        conn.commit()
        cur.close()
        conn.close()
        ...
        import traceback
        logger.error(f"admin_delete_game failed: {e}\n{traceback.format_exc()}")
        return jsonify({"error": str(e), "type": type(e).__name__}), 500
