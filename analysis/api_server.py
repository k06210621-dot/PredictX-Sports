...
def admin_fix_npb_pairing():
    ...
    token = request.headers.get('X-Admin-Token', '')
    if token != 'predictx-fix-npb-2026-06-19':
        return jsonify({"error": "unauthorized"}), 403

    # 錯誤的 game_ids（樂天 vs 西武、歐力士 vs 羅德）
    WRONG_GAME_IDS = [
        "11fed002-d2f2-4096-95da-1dd9b76e1765",  # Rakuten vs Seibu (WRONG)
        "49814d79-ff20-4601-87fd-dc55bfb7c2ab",  # ORIX vs Lotte (WRONG)
    ]
    ...
    try:
        ...
        cur = conn.cursor()
        results = {"deleted": [], "inserted": [], "analyzed": []}

        # Step 1: 刪除錯誤的 2 場（含 game_analysis 連動刪除）
        for gid in WRONG_GAME_IDS:
            cur.execute("DELETE FROM predictx.game_analysis WHERE game_id = %s::uuid", (gid,))
            cur.execute("DELETE FROM predictx.games WHERE game_id = %s::uuid", (gid,))
            results["deleted"].append(gid[:8])
        conn.commit()
        cur.close()
        conn.close()
        ...
        # Step 2: 補入正確的 2 場
        import requests
        cloud_url = os.getenv('CLOUD_API_URL', 'https://predictx-sports-production.up.railway.app').rstrip('/')
        resp = requests.post(
            f"{cloud_url}/api/insert_games",
            json={"games": [
                {
                    "season": 2026,
                    "match_date": "2026-06-19",
                    "home_team": "Chiba Lotte Marines",
                ...
                },
                {
                    "season": 2026,
                    "match_date": "2026-06-19",
                    "home_team": "ORIX Buffaloes",
                    "away_team": "Saitama Seibu Lions",
                    "status": "SCHEDULED",
                },
            ]},
            timeout=30,
        )
        results["insert_resp"] = resp.json()

        # Step 3: 查詢剛插入的 game_id 並觸發 AI 分析
        # 透過 API 查
        games_resp = requests.get(f"{cloud_url}/api/games?league=NPB&days=2", timeout=20).json()
        correct_ids = []
        for g in games_resp:
            if (g['match_date'] == '2026-06-19' and
                g['home_team'] in ['Chiba Lotte Marines', 'ORIX Buffaloes']):
                correct_ids.append(g['game_id'])
                results["inserted"].append(g['game_id'][:8])

        # Step 4: 對正確的 2 場跑 AI 分析
        engine = AnalysisEngine()
        for gid in correct_ids:
            try:
                result = engine.analyze_game(gid)
                if result:
                    engine.cur.execute(
                        """INSERT INTO predictx.game_analysis (game_id, analysis_data, updated_at)
                           ...
                           ON CONFLICT (game_id)
                           DO UPDATE SET analysis_data = EXCLUDED.analysis_data, updated_at = CURRENT_TIMESTAMP""",
                        (gid, __import__('json').dumps(result, ensure_ascii=False))
                    )
                    engine.conn.commit()
                    results["analyzed"].append(gid[:8])
            except Exception as e:
                results.setdefault("analyze_errors", []).append(f"{gid[:8]}: {str(e)[:100]}")
        engine.close()
        ...
        logger.error(f"admin_fix_npb_pairing: {tb}")
